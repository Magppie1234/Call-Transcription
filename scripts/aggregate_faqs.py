...
CLUSTER_PROMPT = """These are questions customers asked on sales calls for Magppie, a premium modular kitchen company in India. All are on the topic "{topic}". Many are the same underlying question phrased differently.
...
DRAFT_PROMPT = """You are preparing a sales-training answer bank for Magppie, a premium modular kitchen company in India. Below are customer FAQs that agents frequently failed to answer clearly on real calls, each with whatever GOOD answers other agents did give elsewhere (verbatim or summarised from real calls).
