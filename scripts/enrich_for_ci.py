...
class Readiness(BaseModel):
    need_fit: int = Field(description="0-100: how well a stated need matches what Magppie sells")
...
class Enrichment(BaseModel):
    ...
    product_mentioned: Optional[str] = Field(None, description="Magppie product/range actually named in the call; null if none")
...
PROMPT = """You are analysing one sales call for Magppie, a premium STONE modular-kitchen company in India. The diarised transcript is romanised Hindi/Hinglish mixed with English.

Speaker roles are decided by CONTENT, not by label: the SALESPERSON pitches Magppie, asks about the customer's house/site/budget and offers visits; the CUSTOMER is being sold to.
...
def fetch_summaries():
    ...
    cols = ("call_id,agent,customer,call_outcome,customer_sentiment,interest_level,summary,"
            ...
            "timeline,kitchen_type,stakeholders,action_items,analysis,duration_seconds")
    while True:
...
def build_context(summ, faq_questions):
    ...
    return json.dumps({
        ...
        "kitchen_type": summ.get("kitchen_type"),
