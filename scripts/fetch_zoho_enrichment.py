...
LEAD_FIELDS = ("id,Lead_Source,Lead_Status,City,State1,Zip_Code,Lead_Type,Product_Requirement,"
               "Project_Stage,Type_of_Client,Property_Type,Lead_Ratings,Campaign_Name,"
               "utm_source,utm_campaign,Company,Email,Phone")
CONTACT_FIELDS = ("id,Lead_Source,Mailing_City,Mailing_State1,Mailing_Zip,Other_City,"
                  "Account_Name,Email,Phone")
DEAL_FIELDS = ("id,Deal_Name,Stage,Lead_Source,Amount,Total_Amount,Product_Name,Product_Type,"
               "Kitchen_Type,Wardrobe_Type,Type_of_Client,Property_Type,Project_Stage,"
               "city,State,Zip_Code,Closing_Date,Contact_Name")
...
def main():
    ...
    for cid in call_ids:
        ...
        if r:
            ...
            if module == "Leads" and rid in leads:
                L = leads[rid]
                row.update(
                    lead_source=pick(L, "Lead_Source", "utm_source"),
                    campaign=pick(L, "Campaign_Name", "utm_campaign"),
                    crm_stage=pick(L, "Lead_Status", "Project_Stage"),
                    pincode=pick(L, "Zip_Code"),
                    client_type=pick(L, "Type_of_Client", "Lead_Type"),
                    property_type=pick(L, "Property_Type"),
                    product_requirement=pick(L, "Product_Requirement"),
                    lead_rating=pick(L, "Lead_Ratings"))
                ...
                if D:
                    row.update(has_deal=True,
                               ...
                               deal_amount=pick(D, "Total_Amount", "Amount"),
                               deal_product=pick(D, "Product_Name", "Kitchen_Type", "Product_Type"),
                               crm_stage=pick(D, "Stage", "Project_Stage"),
                               client_type=pick(D, "Type_of_Client"),
                               property_type=pick(D, "Property_Type"),
                               pincode=row["pincode"] or pick(D, "Zip_Code"))
        results[cid] = row
    ...
    for k in ["linked_module", "lead_source", "campaign", "crm_stage", "pincode",
              "client_type", "property_type", "product_requirement", "lead_rating",
              "has_deal", "deal_stage", "deal_amount"]:
        print("   " + cov(k))
