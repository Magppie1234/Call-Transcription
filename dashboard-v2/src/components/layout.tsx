...
const NAV: { path: string; label: string; icon: string; group: string }[] = [
  ...
  { path: '/summaries', label: 'Call Summaries', icon: '¶', group: 'Performance' },
  ...
  return (
    <div className="topbar">
      <div className="filterbar">
        ...
        <Select value={filters.employee} onChange={set('employee')} options={EMPLOYEES.map((e) => [e.id, e.name.split(' ')[0] + ' ' + e.name.split(' ')[1]?.slice(0, 1) + '.'] as [string, string])} placeholder="All employees" label="Employee" />
